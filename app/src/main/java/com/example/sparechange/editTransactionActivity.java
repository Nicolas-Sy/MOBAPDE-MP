    ...
    public void updateTransaction(View v) {
        ...
        if (!(TextUtils.isEmpty(name) && TextUtils.isEmpty(type))) {
            ...
            if (type.equals("Expense")) {
                Transaction transaction = new Transaction(databaseID, name, type, category, amount, transacDate, userID);
                ...
                Transaction transaction = new Transaction(databaseID, name, type, category, amount2, transacDate, userID);
                databaseTransactions.child(databaseID).setValue(transaction);
                Intent intent = new Intent(getApplicationContext(), MainActivity.class);
                intent.putExtra(TRANSACTION_NAME, name);
                intent.putExtra(TRANSACTION_ID, databaseID);
